...
- ONE-ENTITLEMENT-PER-SLOT IS VENUE DISCIPLINE, NOT A LEDGER GUARANTEE. Without contract keys nothing prevents the operator from issuing two entitlements for the same concurrencyIndex and doubling the effective limit. Accepted as irreducible at LF 2.1; the mitigation is that every issuance is observable by the player and the auditor, and the published report must expose issuance lineage so a discrepancy is detectable after the fact.

- A LOST PlayerEntitlement LOCKS A PLAYER OUT ENTIRELY — no key, no lookup, and at concurrencyLimit 1 no spare. Recovery paths exist (GameStake_ExpireUnsettled from the player's side, GameVenue_IssueEntitlements from the operator's) but they are ops procedures, not automatic. Accepted; a metric for players with zero entitlements and no open stake is a launch requirement, not a nice-to-have.
